...
func (bb *BitBoard) placeFoodRandomly(num int) {
	for i := 0; i < num; i++ {
		...
		if len(unoccupiedIndices) > 0 {
			...
			bb.food.SetBit(bb.food, newFood, 1)
			bb.totalFood += 1
		}
	}
}
...
func (bb *BitBoard) placeFoodFixed() {
	...
	for _, snake := range bb.Snakes {
		...
		for _, p := range possibleFoodLocations {
			...
			coordBoard := big.NewInt(0)
			coordBoard.SetBit(coordBoard, coordIndex, 1)

			// skip if there is already food
			if coordBoard.And(coordBoard, bb.food).BitLen() > 0 {
				continue
			}
		...
		bb.food.SetBit(bb.food, getIndex(placedFood, bb.width), 1)
	}

	isCenterOccupied := true
	centerBoard := big.NewInt(0)
	centerBoard.SetBit(centerBoard, getIndex(centerCoord, bb.width), 1)

	if centerBoard.And(centerBoard, bb.empty).BitLen() == 0 {
	...
	bb.food.SetBit(bb.food, getIndex(centerCoord, bb.width), 1)
}
